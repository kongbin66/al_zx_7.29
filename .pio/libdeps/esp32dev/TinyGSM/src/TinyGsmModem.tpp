...
template <class modemType>class TinyGsmModem {
 public:
  /*
   * Basic functions
   */
  ...
  bool sleepEnable(uint8_t enable = true) {
 ...
 protected:
  ...
  inline modemType& thisModem() {
    return static_cast<modemType&>(*this);//强制类型转换
  }
 ...
 protected:
  ...
  inline int16_t streamGetIntBefore(char lastChar) {
    char   buf[7];
    size_t bytesRead = thisModem().stream.readBytesUntil(lastChar, buf, static_cast<size_t>(7));
  ...
  {
    char   buf[16];
    size_t bytesRead = thisModem().stream.readBytesUntil( lastChar, buf, static_cast<size_t>(16));
    //size_t bytesRead = this.stream.readBytesUntil( lastChar, buf, static_cast<size_t>(16));
